...
### V1.0.3 新增功能
- **Multipart 请求体**：`MultipartBody` 构建器，支持 multipart/form-data 文件上传（文本字段、byte[]、Path、InputStream）
- **CORS 配置**：`CorsConfig` 不可变记录 + Builder，来源/方法/头部白名单，预设配置（`allowAll()`、`restrictive()`）
- **安全头部**：`SecurityHeaders` 构建器，支持 CSP、HSTS、X-Frame-Options、X-Content-Type-Options、Referrer-Policy、Permissions-Policy、COEP、COOP
- **限流信息**：`RateLimitInfo` 记录，解析/构建 X-RateLimit-* 和 Retry-After 头部
- **问题详情 (RFC 9457)**：`ProblemDetail` 记录，标准错误响应格式，支持扩展属性
- **媒体类型**：`MediaType` 记录，Accept 头部解析、质量因子排序、内容协商（`bestMatch`）
- **ETag**：`ETag` 记录，基于 SHA-256 的 ETag 生成、强/弱比较、If-None-Match 匹配（RFC 7232）
- **客户端 IP 解析**：`ClientIp` 工具类，从 X-Forwarded-For、X-Real-IP、CF-Connecting-IP、True-Client-IP 解析真实客户端 IP，支持信任代理过滤和 IPv4/IPv6
- **异常体系**：`OpenWebException` 现继承 `OpenException`（核心统一异常基类）
...
| 类 | 说明 |
|----|------|
| `ProblemDetail` | RFC 9457 HTTP API 问题详情，支持 Builder 和扩展属性 |

### 限流 (`web.ratelimit`)
| 类 | 说明 |
|----|------|
| `RateLimitInfo` | 解析/构建 X-RateLimit-* 和 Retry-After 头部 |

### 安全 (`web.security`)
| 类 | 说明 |
|----|------|
| `SecurityHeaders` | 安全响应头部构建器（CSP、HSTS、X-Frame-Options、X-Content-Type-Options、Referrer-Policy 等） |
