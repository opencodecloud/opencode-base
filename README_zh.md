...
OpenCode Base 是一个包含 45 个模块化组件的综合工具套件，覆盖核心工具、数据处理、安全加密、并发处理、机器学习推理和业务逻辑——全部基于现代 Java 特性构建，包括虚拟线程、Record、密封类和模式匹配。
...
```
opencode-base（45 个模块）
┌─────────────────────────────────────────────────────────────────┐
│                     业务组件 (3xx) — 15 个                       │
│  验证码  邮件  事件  功能开关  地理  图  图片  农历               │
│  货币  测试  时序  树  Web  PDF  Cron                            │
├─────────────────────────────────────────────────────────────────┤
│                     领域组件 (2xx) — 12 个                       │
│  配置  CSV  函数式  JSON  日志  并行  对象池                     │
│  序列化  XML  YAML  OAuth2  神经网络                             │
├─────────────────────────────────────────────────────────────────┤
│                     基础组件 (1xx) — 17 个                       │
│  缓存  类加载  集合  加密  日期  深克隆  哈希                    │
│  国际化  ID  IO  数学  反射  字符串  表达式  锁  规则  可观测    │
├─────────────────────────────────────────────────────────────────┤
│                     核心组件 (0xx) — 1 个                        │
│                          Core                                    │
└─────────────────────────────────────────────────────────────────┘
...
#### math — 科学计算
线性代数（`Vector`、`Matrix`）、概率分布（正态、T、卡方、F、Beta、Gamma、对数正态、二项式）、数值分析（Brent 求根、Simpson 积分、数值微分）、插值、组合数学（二项式系数、排列）、特殊函数（gamma、beta、erf）、描述性统计（含流式累加器）、假设检验（T 检验、卡方检验、方差分析）、回归分析、Spearman/Kendall 相关性。
...
#### csv — CSV 处理
RFC 4180 兼容的解析器/写入器，内置 DoS 和公式注入防护。对象绑定（CSV ↔ Record/POJO，通过 `@CsvColumn`/`@CsvFormat` 注解）、流式 `CsvReader`/`CsvWriter`（低内存占用）、文档差异对比、合并（纵向拼接/内连接/左连接）、流式查询/过滤/投影、列统计、行采样、分块拆分、转换管道、规则验证。
...
#### neural — 神经网络推理
自定义 `.ocm` 二进制模型格式（含计算图）。`Tensor`（float32、步幅寻址、零拷贝视图）、约 30 个算子（Conv1D/2D、DepthwiseConv2D、MaxPool/AvgPool、LSTM、BiLSTM、CTC 解码/束搜索、BatchNorm、Dropout、Linear、全部标准激活函数）、`InferenceSession` 模型推理、损失函数（MSE、MAE、交叉熵、Huber、余弦相似度）、评估指标（准确率、精确率、召回率、F1、混淆矩阵）、输入归一化（MinMax、Z-Score、L2）、权重初始化、量化支持。
