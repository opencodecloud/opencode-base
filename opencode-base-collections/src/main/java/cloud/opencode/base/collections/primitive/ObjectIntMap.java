 ...
 *
 * <p>A map with object keys and primitive int values to avoid value boxing overhead.</p>
 * <p>具有对象键和原始 int 值的映射，以避免值装箱开销。</p>
 *
...
public final class ObjectIntMap<K> implements Serializable {
    ...
    private ObjectIntMap(int initialCapacity) {
     ...
     *
     * @param <K> key type | 键类型
     * @return new empty ObjectIntMap | 新空 ObjectIntMap
     */
    public static <K> ObjectIntMap<K> create() {
        return new ObjectIntMap<>(DEFAULT_CAPACITY);
    }

    /**
     * Create an ObjectIntMap with initial capacity.
     * 创建指定初始容量的 ObjectIntMap。
     *
     * @param <K>             key type | 键类型
     * @param initialCapacity initial capacity | 初始容量
     * @return new empty ObjectIntMap | 新空 ObjectIntMap
     */
    public static <K> ObjectIntMap<K> create(int initialCapacity) {
        return new ObjectIntMap<>(initialCapacity);
    }
     ...
     */
    public void addTo(K key, int increment) {
        Objects.requireNonNull(key, "Key cannot be null");
        if (size >= threshold) {
            resize();
        }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof ObjectIntMap<?> that)) return false;
        if (size != that.size) return false;
        @SuppressWarnings("unchecked")
        ObjectIntMap<K> other = (ObjectIntMap<K>) that;
        for (int i = 0; i < keys.length; i++) {
            if (used[i]) {
                @SuppressWarnings("unchecked")
                K key = (K) keys[i];
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < keys.length; i++) {
            if (used[i]) {
                result += keys[i].hashCode() ^ Integer.hashCode(values[i]);
            }
        }
        return result;
    }
