...
/**
 * X448 key exchange engine (Curve448 ECDH) - Higher security level than X25519
 * X448 密钥协商引擎 - 比 X25519 提供更高的安全级别
 *
 * <p><strong>Features | 主要功能:</strong></p>
 * <ul>
 *   <li>X448 Diffie-Hellman key exchange - X448 Diffie-Hellman 密钥交换</li>
 *   <li>448-bit security level - 448 位安全级别</li>
 * </ul>
 *
 ...
 */
public final class X448Engine implements KeyExchangeEngine {

    private static final String ALGORITHM = "X448";
    private static final String KEY_ALGORITHM = "X448";
    ...
    private X448Engine(KeyPair keyPair) {
        this.privateKey = keyPair.getPrivate();
    }

    /**
     * Creates a new X448 engine instance
     * 创建新的 X448 引擎实例
     *
     * @return new X448 engine instance
     */
    public static X448Engine create() {
        return new X448Engine();
    }

    /**
     * Creates a new X448 engine instance with a generated key pair
     * 创建新的 X448 引擎实例并生成密钥对
     *
     * @return new X448 engine instance with generated key pair
     */
    public static X448Engine withGeneratedKeyPair() {
        X448Engine engine = new X448Engine();
        KeyPair keyPair = engine.generateKeyPair();
        return new X448Engine(keyPair);
    }

    /**
     * Performs X448 key agreement between two parties (static method)
     * 执行两方之间的 X448 密钥协商（静态方法）
     *
     * @param myPrivateKey the local private key
     * @param theirPublicKey the remote public key
     * @return the shared secret (56 bytes)
    ...
    /**
     * Performs X448 key agreement and derives key material using HKDF (static method)
     * 执行 X448 密钥协商并使用 HKDF 派生密钥材料（静态方法）
     *
     * @param myPrivateKey the local private key
     * @param theirPublicKey the remote public key
    ...
    public static byte[] agreeAndDerive(PrivateKey myPrivateKey, PublicKey theirPublicKey,
        ...
        try {
            return Hkdf.sha512().deriveKey(sharedSecret, info, keyLength);
        } finally {
    ...
    public KeyPair generateKeyPair() {
        try {
            ...
            keyPairGenerator.initialize(448, new SecureRandom());
    ...
    public byte[] computeSharedSecret() {
        if (privateKey == null) {
            throw OpenKeyException.keyNotSet("X448 agreement (private key missing)");
        }
        if (remotePublicKey == null) {
            throw OpenKeyException.keyNotSet("X448 agreement (remote public key missing)");
        }
    ...
    public byte[] deriveKey(byte[] info, int length) {
        ...
        try {
            return Hkdf.sha512().deriveKey(sharedSecret, info, length);
        } finally {
            // Clear sensitive data
            java.util.Arrays.fill(sharedSecret, (byte) 0);
        }
     ...
     * 从私钥获取公钥（如果已设置）。
     *
     * <p>X448 does not support deriving a public key from an existing private key
