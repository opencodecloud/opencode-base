...
public final class SafePathUtil {
    ...
    public static void validatePath(Path path) throws ImageValidationException {
        ...
        // Check ALL path components for traversal patterns, not just filename
        String rawPath = path.toString();
        if (rawPath.contains("..")) {
            throw new ImageValidationException("Invalid path: contains path traversal pattern");
        }

        String fileName = path.getFileName().toString();

        // Check for path traversal patterns via substring containment on filename
